...
/**
 * Adds the Daily Dev Habit admin menu pages.
 */
function devhabit_add_admin_menu() {
    ...
    add_submenu_page(
        'daily-devhabit',                                // Parent Slug
        __( 'Daily DevHabit Settings', 'daily-devhabit' ),    // Page Title
    ...
    <div class="wrap daily-devhabit-page-wrapper">
        ...
        <div class="daily-devhabit-intro-text" style="max-width: 700px; margin: 1.5em auto 1.5em auto; text-align: center;">
            ...
            <p style="margin-bottom: 0.5em; color: #50575e;">
                ...
                <?php esc_html_e( 'Tip: Use Ctrl+Enter (Cmd+Enter on Mac) to quickly advance.', 'daily-devhabit' ); ?>
            </p>
            <p style="margin-bottom: 0;"> <?php // Remove bottom margin on last paragraph ?>
                ...
                <?php printf(
                    wp_kses_post( __( 'Configure your <a href="%s">GitHub Settings</a> to enable auto-saving.', 'daily-devhabit' ) ),
...
/**
 * Renders the HTML for the Daily Dev Habit settings page.
 */
function devhabit_render_settings_page() {
    ...
    <div class="wrap">
        ...
        <form method="post" action="options.php">
            ...
            submit_button( __( 'Save Settings', 'daily-devhabit' ) );
            ?>
        </form>
    </div>
    <?php
...
function devhabit_settings_init() {
    ...
    add_settings_section(
        ...
        __( 'GitHub Integration Settings', 'daily-devhabit' ), // Title
        'devhabit_github_section_callback',             // Callback to render section description
        'daily-devhabit-settings'                          // Page slug where this section appears
    );

    // Add fields to the section
    add_settings_field(
        'github_pat',                                   // Field ID
        __( 'GitHub Personal Access Token', 'daily-devhabit' ), // Field Title
        'devhabit_render_github_pat_field',             // Callback to render the input
        'daily-devhabit-settings',                         // Page slug
        'devhabit_github_section'                       // Section ID
    );

    add_settings_field(
        'github_username',
        __( 'GitHub Username', 'daily-devhabit' ),
     ...
     add_settings_field(
        'github_repo',
        __( 'Target Repository Name', 'daily-devhabit' ),
     ...
     add_settings_field(
        'github_path',
        __( 'Default File Path (Optional)', 'daily-devhabit' ),
...
function devhabit_github_section_callback() {
    echo '<p>' . esc_html__( 'Configure your GitHub settings to automatically save logs.', 'daily-devhabit' ) . '</p>';
    echo '<p>' . wp_kses_post( __( 'You need to create a <a href="https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens" target="_blank" rel="noopener noreferrer">Personal Access Token</a> with the <code>public_repo</code> or <code>repo</code> scope.', 'daily-devhabit' ) ) . '</p>';
...
function devhabit_render_github_pat_field() {
    ...
    <p class="description"><?php esc_html_e( 'Your GitHub PAT. This is stored securely but treated like a password.', 'daily-devhabit' ); ?></p>
    <?php
}
...
function devhabit_render_github_repo_field() {
    ...
    <p class="description"><?php esc_html_e( 'The name of the repository where logs should be saved.', 'daily-devhabit' ); ?></p>
    <?php
}
...
function devhabit_render_github_path_field() {
    ...
    <p class="description"><?php esc_html_e( 'The folder within the repository to save logs. Must end with a slash if specified. Defaults to _logs/.', 'daily-devhabit' ); ?></p>
    <?php
}
...
function devhabit_enqueue_admin_scripts( $hook ) {
    if ( 'toplevel_page_daily-devhabit' !== $hook && 'dev-habit-log_page_daily-devhabit-settings' !== $hook) {
        return;
    }
    ...
    if ( 'toplevel_page_daily-devhabit' === $hook ) {
        wp_enqueue_script(
            'daily-devhabit-admin-js',
            $plugin_url . 'admin.js',
            [], // Dependencies
            $plugin_version,
            true // Load in footer
        ...
        wp_localize_script('daily-devhabit-admin-js', 'devhabit_ajax', array(
...
function devhabit_ajax_save_log_to_github() {
    ...
    if ( is_wp_error( $response ) ) {
        ...
        if ( $status_code === 201 || $status_code === 200 ) { // 201 = Created, 200 = Updated 
...
function devhabit_add_admin_footer_link( $footer_text ) {
    ...
    // Check if we are on one of the Daily Dev Habit pages
    if ( $screen && ( $screen->id === 'toplevel_page_daily-devhabit' || $screen->id === 'dev-habit-log_page_daily-devhabit-settings' ) ) {
        $support_url = 'https://daily-devhabit.com/support/';

        // Append your link to the existing footer text
        $footer_text .= ' | ' . sprintf(
            wp_kses_post( __( 'Need help? Visit the <a href="%s" target="_blank" rel="noopener noreferrer">Daily Dev Habit Support</a> page.', 'daily-devhabit' ) ),
            esc_url( $support_url )
        );
    }
